...
# Build space UUID → label mapping
echo ""
echo "Building space UUID to label mapping..."
SPACE_UUID_MAPPINGS=()

# Re-query spaces to get their UUIDs after labeling
UPDATED_SPACES=$(yabai -m query --spaces)

for label in "${LAPTOP_LABELS[@]}" "${LEFT_LABELS[@]}" "${RIGHT_LABELS[@]}"; do
    # Find the space with this label and get its UUID
    space_uuid=$(echo "$UPDATED_SPACES" | jq -r --arg label "$label" '.[] | select(.label == $label) | .uuid')
    if [[ -n "$space_uuid" && "$space_uuid" != "null" ]]; then
        SPACE_UUID_MAPPINGS+=("$space_uuid:$label")
        echo "  $space_uuid → $label"
    fi
done
...
echo ""
echo "# Space UUID to label mapping (for relabeling after monitor disconnect)"
echo "SPACE_UUID_LABELS=(${SPACE_UUID_MAPPINGS[*]})"
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    # Update or add SPACE_UUID_LABELS mapping
    if grep -q "^SPACE_UUID_LABELS=" "$CONFIG_FILE"; then
        sed -i.bak "s/^SPACE_UUID_LABELS=.*/SPACE_UUID_LABELS=(${SPACE_UUID_MAPPINGS[*]})/" "$CONFIG_FILE"
    else
        # Add the mapping section if it doesn't exist
        echo "" >> "$CONFIG_FILE"
        echo "# -----------------------------------------------------------------------------" >> "$CONFIG_FILE"
        echo "# SPACE UUID TO LABEL MAPPING (auto-generated by init-spaces.sh)" >> "$CONFIG_FILE"
        echo "# -----------------------------------------------------------------------------" >> "$CONFIG_FILE"
        echo "# Used by restore-spaces.sh to relabel spaces after monitor disconnect" >> "$CONFIG_FILE"
        echo "SPACE_UUID_LABELS=(${SPACE_UUID_MAPPINGS[*]})" >> "$CONFIG_FILE"
    fi
